...
@section('head')
  <title>Purein - {{$field -> art_title}}</title>
	...
	<div class="container">
		...
		<div class="row">
			<div class="col-md-9 about center">       
                      ...
                      <ul class="breadcrumb">
                        <li>{{$cate_name}}</li>
                        <li class="active"><a title="{{$field -> art_title}}" href="{{url('c/'.$field->cate_id)}}">{{$field -> cate_name}}</a></li>
                      </ul>                        
          ...
          <div class="row">
            <div class="col-md-9 center">
              ...
              <li>上一篇 : 
                    @if($article['pre'])
                        <a title="{{$article['pre']->art_title}}" href="{{url('a/'.$article['pre']->art_id)}}">{{$article['pre']->art_title}}</a>
                    @else
                        <span>没有上一篇了</span>
                    @endif
              </li>
              ...
              <li>下一篇 : 
                    @if($article['next'])
                        <a title="{{$article['next']->art_title}}" href="{{url('a/'.$article['next']->art_id)}}">{{$article['next']->art_title}}</a>
                    @else
                        <span>没有下一篇了</span>
                    @endif
              </li>
